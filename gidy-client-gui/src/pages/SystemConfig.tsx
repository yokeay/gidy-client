...
import { getConfig, updateConfig, generatePsk, GuiConfig } from "../api";
...
export default function SystemConfig() {
  ...
  const inputClass = "w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50";
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-sm">{t("systemConfig.proxyServer")}</h3>
          </div>
          <div className="space-y-4">
            <div>
              <label className={labelClass}>{t("systemConfig.serverAddr")}</label>
              <input
            ...
            <div>
              <label className={labelClass}>{t("systemConfig.serverPort")}</label>
              <input
                className={inputClass}
                value={config.server_port}
                onChange={e => handleChange("server_port", parseInt(e.target.value) || 0)}
                placeholder="443"
              />
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs text-muted-foreground">{t("systemConfig.psk")}</label>
                <button
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-sm">{t("systemConfig.localProxy")}</h3>
          </div>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{t("systemConfig.socks5Addr")}</label>
                <input
                  className={inputClass}
              ...
              <div>
                <label className={labelClass}>{t("systemConfig.socks5Port")}</label>
                <input
                  className={inputClass}
                  value={config.socks5_port}
                  onChange={e => handleChange("socks5_port", parseInt(e.target.value) || 0)}
                  placeholder="1080"
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>{t("systemConfig.httpAddr")}</label>
                <input
                  className={inputClass}
              ...
              <div>
                <label className={labelClass}>{t("systemConfig.httpPort")}</label>
                <input
                  className={inputClass}
                  value={config.http_port}
                  onChange={e => handleChange("http_port", parseInt(e.target.value) || 0)}
                  placeholder="8080"
                />
              </div>
            </div>
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-semibold text-sm">{t("systemConfig.connectionNote")}</h3>
        </div>
      ...
      <div className="flex items-center gap-4">
        ...
        {message && (
          <span className={`text-xs ${message.type === "success" ? "text-emerald-500" : "text-red-500"}`}>
            {message.text}
          </span>
        )}
      </div>
